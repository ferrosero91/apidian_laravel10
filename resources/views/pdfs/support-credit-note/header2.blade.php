    ...
    <tbody>
        <tr>
            <td style="width: 30%; text-align: center;">
                <p style="font-weight: bold; font-size: 11px;">NOTA DE AJUSTE DEL DOCUMENTO SOPORTE A NO OBLIGADOS A FACTURAR No</p>
                <p style="color: red;
                    font-weight: bold;
                    font-size: 16px;
                    text-align: center;
                ...
                <div id="empresa-header1">
                    @if(isset($request->ivaresponsable))
                        @if($request->ivaresponsable != $company->type_regime->name)
                            NIT: {{$company->identification_number}}-{{$company->dv}} - {{$company->type_regime->name}} - {{$request->ivaresponsable}} - Obligacion: {{$company->type_liability->name}}
                        @else
                            NIT: {{$company->identification_number}}-{{$company->dv}} - {{$company->type_regime->name}} - Obligacion: {{$company->type_liability->name}}
                        @endif
                    @else
                        NIT: {{$company->identification_number}}-{{$company->dv}} - {{$company->type_regime->name}} - Obligacion: {{$company->type_liability->name}}
                    @endif
                    ...
                    @if(isset($request->seze))
                        ...
                        ?>
                       Regimen ZESE Año: {{$aseze}} Constitucion Sociedad Año: {{$asociedad}}<br>
                    @endif
                    REPRESENTACION GRAFICA DE NOTA CREDITO DE DOCUMENTO SOPORTE ELECTRONICO A NO OBLIGADOS<br>
